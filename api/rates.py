...
def fetch_rate(bank_slug: str, currency: str) -> dict:
    """Fetch buy/sell rate for a bank from doviz.com"""
    currency_map = {
        "USD": "amerikan-dolari",
        "EUR": "euro",
        "RUB": "rus-rublesi",
    ...
    try:
        ...
        # Extract sell rate (satış) - what customer pays to buy currency
        # Pattern: "Alış / Satış" followed by numbers
        ...
        # Fallback: look for JSON-LD or meta with price
        pattern2 = r'"price"\s*:\s*"?([\d,\.]+)"?'
        match2 = re.search(pattern2, html)
        if match2:
            price = float(match2.group(1).replace(",", "."))
            return {"buy": round(price * 0.96, 4), "sell": price}
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        # Parse currency from path: /api/rates?currency=USD
        currency = "USD"
        ...
        # Sort by sell rate ascending (cheapest first)
